...
(async function () {
  ...
  const NEW_DAYS = 7;
  const LOW_STOCK = 3;
  const WISHLIST_KEY = 'elyna_wishlist';
  let items = [];
  let settings = {};
  let suspended = false;
  ...
  const ANALYTICS_KEY = 'elyna_analytics';
  ...
  function enquireBody(item, soldOut, selectedSize) {
    ...
    return soldOut
      ? `Hi Elyna! I saw *${item.name}* is sold out. Will it be back in stock? I'd love to reserve one.`
      : `Hi Elyna! I'd like to enquire about *${item.name}*${sizePart}${pricePart} from your catalog.`;
  }

  function whatsappLink(item, soldOut, selectedSize) {
    const phone = settings.whatsappNumber || '254794406221';
  ...
  document.getElementById('wishlistEnquireAll')?.addEventListener('click', e => {
    ...
    const msg = `Hi Elyna! I'd like to enquire about these saved items:\n\n${lines.join('\n')}\n\nAre they available?`;
  ...
  function showSuspended() {
    ...
    const shopName = settings.shopName || settings.businessName || 'Elyna Footwear';
    const tagline = settings.tagline || 'Brand new shoes';
    const igHandle = (settings.instagramHandle || 'elynafootwear').replace(/^@/, '');
