...
function checkAuth() {
  if (sessionStorage.getItem('elyna_auth') === '1') {
...
function generateDescription(name, cat) {
  ...
  const catMap = {
    Flats: 'pair of flats', Heels: 'pair of heels', Sandals: 'pair of sandals',
    Wedges: 'pair of wedges', Mules: 'pair of mules', Loafers: 'pair of loafers',
    Slides: 'pair of slides', Sneakers: 'pair of sneakers', Boots: 'pair of boots',
    Official: 'pair of official shoes', Shoes: 'pair of shoes',
  };
  const type = catMap[cat] || 'pair';

  const openers = [
    `Step out in these ${color || 'quality'} ${type}, brand new and ready to wear.`,
    `A clean ${color || 'versatile'} ${type} that pairs with everything in your wardrobe.`,
    `${color ? color.charAt(0).toUpperCase() + color.slice(1) : 'Premium'} ${type}, brand new stock. Comfortable, durable, made to turn heads.`,
  ];
  const mids = [
  ...
  const closes = [
    ...
    `Pick up at Sawa Mall, Shop C21, Moi Avenue, or we deliver.`,
  ];
...
window.clientMessage = phone => {
  ...
  const msg = `Hi ${first}! Thanks for shopping with Elyna Footwear. Fresh pieces just landed. Want me to send you what's new?`;
...
function buildBroadcastMessage(recipientName) {
  ...
  const itemsBlock = items.length
    ...
    : '';
  const lookUrl = 'https://elynafootwear.essenceautomations.com';
  const greet = recipientName ? `Hi ${recipientName.split(' ')[0]}! ` : 'Hi! ';
  return `${greet}It's Elyna Footwear — ${subject || 'fresh stock just landed'}.${itemsBlock}\n\nTap to browse: ${lookUrl}\n\nReply here to enquire. 🤍`;
}
...
const BC_PROG_KEY = 'elyna_bcprog';
...
const INSIGHTS_KEY = 'elyna_analytics';
...
const IG_USER_ID = '48825175757';
const FOOTWEAR_CATEGORIES = ['Flats', 'Heels', 'Sling Backs', 'Sandals', 'Wedges', 'Mules', 'Loafers', 'Slides', 'Sneakers', 'Boots', 'Official', 'Shoes'];
...
function renderIgSyncList() {
  igSyncListEl.innerHTML = igSyncCandidates.map((it, i) => {
    ...
    // Elyna is NEW-STOCK: stock is { "M":1, "L":1, "XL":1 } etc. Show every
    ...
    const catOpts = FOOTWEAR_CATEGORIES.map(c => `<option value="${c}" ${c === s.category ? 'selected' : ''}>${c}</option>`).join('');
    return `
